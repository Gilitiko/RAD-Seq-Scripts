...
def t_diff(lines, src, dst, filt, sample_index):
    ...
    for i in range(1, len(lines)):
        if len(lines[i][src]) == 0 or len(lines[i][dst]) == 0 or lines[i][src] == b"--" or lines[i][dst] == b"--" or lines[i][dst] == b"./." or lines[i][src] == b"./.":
            continue
        res.append(abs(int(lines[i][src]) - int(lines[i][dst])))
    if len(res) < filt:
        return ""
    return "{:.2f}".format(sum(res) / float(len(res)))
...
def calculate_distances(lines):
    ...
    for src in tqdm.tqdm(range(sample_index, len(lines[0]))):
        ...
        for dst in range(sample_index, len(lines[0])):
            if src != dst:
                res[lines[0][src]].append(t_diff(lines, src, dst, 0, sample_index))
            else:
...
def filter_lines(snip_filter, sample_filter,input_vcf, limit):
    ...
    out_path = out_path.with_name(out_path.name.replace(".vcf", "_parsed.vcf"))
